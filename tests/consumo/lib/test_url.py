...
def test_get_relative_path_multimedia_duration(
    mock_get_absolute_path_multimedia_duration: MagicMock,
) -> None:
    mock_get_absolute_path_multimedia_duration.return_value = 11
    url: HttpUrl = HttpUrl("https://www.w3schools.com/html/html5_video.asp")
    src: FilePath = Path("mov_bbb.mp4")
    actual_duration: int = get_relative_path_multimedia_duration(url, src)
...
def test_get_multimedia_duration(
    mock_get_relative_path_multimedia_duration: MagicMock,
    mock_get_absolute_path_multimedia_duration: MagicMock,
) -> None:
    mock_get_absolute_path_multimedia_duration.side_effect = ValidationError
    mock_get_relative_path_multimedia_duration.return_value = 11
    url: HttpUrl = HttpUrl("https://www.w3schools.com/html/html5_video.asp")
    src: FilePath = Path("mov_bbb.mp4")
    actual_duration: int = get_multimedia_duration(url, str(src))
    expected_duration: int = 11
